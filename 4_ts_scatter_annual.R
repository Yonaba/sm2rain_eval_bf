...
for (p in products[-1]) {
  ...
  gofs.lab <- paste0(#"R² = ",format(round(gofs[1],2), nsmall=2),
                     "RMSE = ", format(round(gofs[2],1), nsmall=1)," [mm/yr]",
  ...
  dplot <- ggplot(cdf, aes(x=Date, group=1)) +
    ...
    xlab(ifelse(cp==7,"Dates","")) + ylab("Annual rainfall [mm]") + ylim(0,1200) +
    scale_x_discrete(breaks = cdf$Date, labels = cdf$Date) +
  ...
  splot <- ggscatter(cdf, x = "OBS", y = p, size = 2, add = "reg.line", color = "black", shape = 21,
    ...
    xlab(ifelse(cp==7,"Observed [mm/yr]","")) + ylab("Product [mm/yr]") +
    xlim(400,1100) + ylim(400,1100) +
    #stat_regline_equation(formula=y~x+0, label.y = 120,size = 12, na.rm = T) +
    stat_cor(method = "pearson", aes(label = paste(after_stat(r.label),..p.label.., sep="~`,`~")), 
             label.y = 975, size = 5)
...
ggsave(plot = grob, "graphs/ts_scatter_annual.png",
