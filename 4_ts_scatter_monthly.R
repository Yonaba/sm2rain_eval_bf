...
for (p in products[-1]) {
  ...
  dplot <- ggplot(spi.df, aes(x=dates, group=1)) +
    ...
    xlab(ifelse(cp==7,"Dates","")) + ylab("Monthly rainfall [mm]") + ylim(0,350) +
  ...
  splot <- ggscatter(cdf, x = "OBS", y = p, size = 2, add = "reg.line", color = "black", shape = 21,
    ...
    xlab(ifelse(cp==7,"Observed [mm/month]","")) + ylab("Product [mm/month]") +
...
ggsave(plot = grob, "graphs/ts_scatter_monthly.png",
